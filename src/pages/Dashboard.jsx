...
import { MetricCard, ExpenseRow, fmt, CAT_ICONS } from '../components/index.jsx'

ChartJS.register(ArcElement, Tooltip, Legend, CategoryScale, LinearScale, BarElement)

const CAT_COLORS = { Groceries:'#3b82f6', Utilities:'#10b981', Transport:'#f59e0b', Entertainment:'#ec4899', Medical:'#8b5cf6', Construction:'#f97316', Other:'#6b7280' }
...
export default function Dashboard() {
  ...
  const [month, setMonth] = useState(sessionStorage.getItem('gk_month') || '2026-04')
  ...
  useEffect(() => {
    const handler = () => setMonth(sessionStorage.getItem('gk_month') || '2026-04')
  ...
  const catData  = CATS.map(c => expenses.filter(e => e.category === c).reduce((s, e) => s + Number(e.amount), 0))
          ...
          {members.map(m => {
            ...
            return (
              <div key={m.email} className="m-chip">
                ...
                <div><div className="ch-name">{m.name}</div><div className="ch-amt">{fmt(spent, currency)}</div></div>
              </div>
            )
          })}
        </div>
      ...
      <div className="two-col">
        <div className="panel">
          ...
          <div className="legend" style={{ marginBottom: 8 }}>
            {CATS.filter((_, i) => catData[i] > 0).map(c => {
              const pct = total > 0 ? Math.round(catData[CATS.indexOf(c)] / total * 100) : 0
              return <span key={c} className="li"><span className="ldot" style={{ background: CAT_COLORS[c] }} />{c} {pct}%</span>
            })}
          </div>
          <div className="chart-wrap">
            <Doughnut
              data={{ labels: CATS, datasets: [{ data: hasData ? catData : [1], backgroundColor: hasData ? CATS.map(c => CAT_COLORS[c]) : ['#e5e7eb'], borderWidth: 3, borderColor: 'transparent' }] }}
              options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => ` ${currency}${Number(ctx.raw).toLocaleString('en-IN')}` } } } }}
            />
          </div>
        </div>
        <div className="panel">
          ...
          <div className="chart-wrap">
            <Bar
              ...
              options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => ` ${currency}${Number(ctx.raw).toLocaleString('en-IN')}` } } }, scales: { y: { ticks: { callback: v => `${currency}${(v/1000).toFixed(0)}k`, font: { size: 11 } }, grid: { color: 'rgba(128,128,128,.1)' } }, x: { ticks: { font: { size: 11 } }, grid: { display: false } } } }}
            />
          </div>
        </div>
      </div>
        ...
        <div className="panel-hd">
          ...
          <div className="pills">
            {['all','shared','personal','recurring'].map(f => (
              <button key={f} className={`pill${filter===f?' active':''}`} onClick={() => setFilter(f)}>{f.charAt(0).toUpperCase()+f.slice(1)}</button>
            ))}
          </div>
        </div>
        <div className="exp-list">
          {filteredExp.length === 0
            ? <div className="empty">No expenses found. <a onClick={() => navigate('/add')} style={{color:'var(--blue)',cursor:'pointer'}}>Add your first one!</a></div>
