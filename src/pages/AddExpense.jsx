...
export default function AddExpense() {
  ...
  const [form, setForm] = useState({
    desc: '', amount: '', currency: currency, date: todayStr(), time: '',
  ...
  useEffect(() => {
    if (editing) {
      setForm({
        desc: editing.desc || '', amount: editing.amount || '', currency: editing.currency || currency,
        date: editing.date || todayStr(), time: editing.time || '',
        category: editing.category || 'Groceries', paymentType: editing.paymentType || paymentTypes[0],
        paidBy: editing.paidBy || '', shared: editing.shared || 'none',
        recurring: editing.recurring || 'none', notes: editing.notes || '',
      })
    }
  }, [editing])
  ...
  const handlePayTypeChange = (v) => {
    if (v === '__other__') {
      const t = prompt('Enter payment type name:')
      if (t?.trim()) set('paymentType', t.trim())
    } else set('paymentType', v)
  }
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true)
    const expense = { ...form, amount: Number(form.amount), shared: form.shared !== 'none' ? form.shared : null, recurring: form.recurring !== 'none' ? form.recurring : null }
    let ok
    if (isEdit) ok = await editExpense({ ...expense, expenseId: editing.expenseId || id })
    else        ok = await addExpense(expense)
    setSaving(false)
    ...
    <div className="view active">
      <div className="form-card">
        ...
        <div className="scan-hint" onClick={() => navigate('/scan')}>
          <svg viewBox="0 0 18 18" fill="none" width="16" height="16"><rect x="1.5" y="4.5" width="15" height="11" rx="2" stroke="currentColor" strokeWidth="1.5"/><circle cx="9" cy="10" r="2.8" stroke="currentColor" strokeWidth="1.3"/></svg>
          <span>Scan a receipt — OCR auto-fills amount & date</span> →
        </div>

        <div className="fg"><label className="fl">Description *</label><input className="fi" type="text" value={form.desc} onChange={e => set('desc', e.target.value)} placeholder="e.g. Weekly groceries from D-Mart" /></div>
        ...
        <div className="f2">
          ...
          <div className="fg"><label className="fl">Time (optional)</label><input className="fi" type="time" value={form.time} onChange={e => set('time', e.target.value)} /></div>
        </div>
        ...
        <div className="f2">
          ...
          <div className="fg"><label className="fl">Notes (optional)</label><input className="fi" type="text" value={form.notes} onChange={e => set('notes', e.target.value)} placeholder="Any extra details..." /></div>
        </div>

        <div className="f-actions">
          <button className="btn-ghost" onClick={() => navigate(-1)}>Cancel</button>
          <button className="btn-save" onClick={handleSubmit} disabled={saving}>{saving ? (isEdit ? 'Updating...' : 'Saving...') : (isEdit ? 'Update expense' : 'Save expense')}</button>
        </div>
      </div>
    </div>
  )
